...
def get_surgical_data(state:PatientState):
    ...
    response = ExecuteQuery(action="S", query=query, values=values)
    updated_session = dict(state.get("session_state", {}))
    updated_session["surgical_data"] = response
...
def get_past_history_data(state:PatientState):
    ...
    response = ExecuteQuery(action="S", query=query, values=values)
    updated_session = dict(state.get("session_state", {}))
    updated_session["past_history_data"] = response
...
# i) ConnectDB() to be called within ExecuteQuery(). it was observed that external call led to system crash
# ii) Prompt based Execution
...
def ExecuteQuery(action, query, values=None):
    ...
    connect = ConnectDB() # change # 1: ConnectDB() called here
...
# Create the agent and link the tools with it.
...
apikey = ("sk"
 "-HEopabhic4XqYdKz_FjnJMy4hh2YPN290ugnTLopAQdRY_Z9U8tCGe5Gf7txvIiSPQk94QlT3BlbkFJeWIljH1P3Hf3H28pdukd6vXOPtPJVGFBEMPqfNuz4FHHXoSxKQLcpqvydPTNjxFG6rjPE6vJcA")

llm1 = ChatOpenAI(api_key=apikey,temperature=0,model="gpt-4o-mini")
agent1 = create_agent(model=llm1,tools=tools,system_prompt=system_prompt)
